...
def eval_agent_stages(args):
    success_rates = []
    trained_epochs = 1000
    step = 50
    print(f"Searching for agent at {args.agent_path}")
    for i in range(50, trained_epochs+1, step):
        print(f"Rolling out agent at epoch {i}")
        
        # add to the namespace the attribute agent
        args.agent = glob.glob(args.agent_path + f"/model_epoch_{i}*")
        args.agent.sort()
        args.agent = args.agent[0]
        rollout_stats = run_trained_agent(args)
        success_rates.append(rollout_stats["Success_Rate"])
        
    # create save directory for these results as the parent of agent_path
    parent_folder = os.path.dirname(os.path.dirname(args.agent_path))
    agent_results_folder = parent_folder + "/agent_results"
    ...
    np.save(agent_results_folder + "/success_rates.npy", success_rates)
    ...
    plt.savefig(agent_results_folder + "/success_rates.png")
    
    # plot as a histogram, the best success rate against the last 10 suuccess rates
    ...
    plt.figure()
    # add an horizontal line for the best success rate
    plt.axhline(y=best_success_rate, color='r', linestyle='-', label="Best Success Rate")
    bars = plt.bar(range(len(last_10_epochs)), last_10_success_rates, tick_label=last_10_epochs)
    plt.xlabel("Last 10 Success Rates")
    plt.ylabel("Success Rate")
    plt.title("Histogram of Last 10 Success Rates") 
    plt.savefig(agent_results_folder + "/last_10_success_rates_histogram.png")
